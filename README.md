...
Each `init` boots a VM, installs the GitHub Actions runner, registers it (against `--repo` for a repo runner, or `--org` for an org runner), and starts it as a service. The runner name defaults to `<platform>-runner-<6 random chars>`; override with `--name`.
...
- `--repo owner/name` — repository to register against (repo-scoped runner); also accepts full `https://github.com/owner/name` URLs
- `--org orgname` — organization to register against (org-scoped runner; requires `admin:org` on the token)

Exactly one of `--repo` or `--org` is required.
